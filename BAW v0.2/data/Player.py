...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.down = []
        for i in range(4):
            img = pygame.transform.scale(pygame.image.load("assets/objects/sprites/player/sideways_" + str(i+1) + ".png"),(240,160))
            print("loaded player left asset " + str(i))
            self.left.append(img)
            self.image = self.left[i]
            self.rect = self.image.get_rect()
        for i in range(4):
            img = pygame.transform.flip(pygame.transform.scale(pygame.image.load("assets/objects/sprites/player/sideways_" + str(i+1) + ".png"),(240,160)), True, False)
            print("loaded player right asset " + str(i))
            self.right.append(img)
            self.image = self.right[i]
            self.rect = self.image.get_rect()
        for i in range(4):
            img = pygame.transform.scale(pygame.image.load("assets/objects/sprites/player/up_" + str(i+1) + ".png"),(240,160))
            print("loaded player up asset " + str(i))
            self.up.append(img)
            self.image = self.up[i]
            self.rect = self.image.get_rect()
        for i in range(4):
            img = pygame.transform.scale(pygame.image.load("assets/objects/sprites/player/down_" + str(i+1) + ".png"),(240,160))
    ...
    def invEdit(self,item,addOrRemove):
        if addOrRemove == 0:
