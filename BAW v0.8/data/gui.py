...
class run(object):
    def __init__(self,savedData):
        self.titleFont = pygame.font.Font(dir + "assets/gui/fonts/title.ttf", 125)
        self.largeFont = pygame.font.Font(dir + "assets/gui/fonts/normal.ttf", 125)
    ...
    def finalScore(self,playerstats,display):
        self.scoreStr = self.titleFont.render(("Final Score is: " + str(playerstats["score"])), True , self.textColorBlack)
        self.score_image_rect = self.scoreStr.get_rect()
        self.score_image_rect.center = (1500/2 + 5,1000/2 + 5)
        display.blit(self.scoreStr,self.score_image_rect)

        self.scoreStr = self.titleFont.render(("Final Score is: " + str(playerstats["score"])), True , self.textColorWhite)
        self.score_image_rect = self.scoreStr.get_rect()
